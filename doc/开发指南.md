...
└── ui/                                   // 【表现层】Jetpack Compose UI
    ...
    │   └── AboutScreen.kt                 // 关于页面（含打赏功能）
    ├── viewmodel/                         // 状态管理（组合根 + 领域控制器）
    │   ├── MainViewModel.kt              // 组合根：聚合三个状态域暴露给 MainScreen
    │   ├── startup/StartupCoordinator.kt // 启动检查状态机（网络/权限/电池/默认供应商）
...
- **`TalkifyApplication`**: 应用入口类，负责：
  ...
  - 启动时清理旧版本遥测遗留的本地存储（`deleteLegacyTelemetryPrefs()`，抹除 `talkify_tts_telemetry`）
- **`TalkifyExceptionHandler`**: 全局未捕获异常处理器，捕获崩溃后：
  - 发送系统通知提示用户
  - 弹出崩溃对话框（支持"重启应用"和"报告问题"两个选项）；崩溃信息**不作任何上报**，仅写入本地日志
- **`TalkifyAppHolder`**: 全局 Context 持有者，避免供应商层对 Android 组件的直接依赖

#### 3.2.4 无遥测设计（Telemetry-Free）

Talkify **不包含任何遥测、统计或崩溃上报能力**：不会向开发者自建服务或任何第三方分析服务发送使用数据。此前的 Umami 匿名统计与 recorder 会话回放/热图子系统已整体移除——`infrastructure/app/telemetry/` 包（含 `recorder/` 子包）、`ui/components/TelemetryCaptureHost.kt`、`MainActivity` 的路由 pageview 监听、`TalkifyApplication` 的前台上报与 Activity/View 追踪、`TalkifyAppHolder` 的瞬态引用追踪，以及对应单元测试均不再存在于代码库中。

**应用内亦不含版本检查**：启动流程不检查更新，关于页也没有"检查更新"入口。原先的 `infrastructure/app/update/UpdateChecker.kt`（GitHub Release API 版本比对与 ABI 选包）、`UpdateDialog`、`UpdateCheckResult`/`UpdateInfo` 模型、`StartupState.CheckingUpdate`/`UpdateAvailable` 状态及全部相关文案均已删除，版本升级由用户自行从 GitHub Releases 获取（关于页保留开源仓库链接）。

**开发约束**（改动前必读）：

- **不新增埋点**：不要引入任何事件上报门面、统计 SDK（Firebase / Sentry / 友盟等）或会话录制组件
- **不新增联网检查**：不引入版本检查、公告拉取、配置下发等非业务必要的出网行为
- **收敛出网**：应用出网仅限两处业务必需场景——用户自行配置的 TTS 供应商 API、本地模型与音色资源下载
- **不采集画像**：不读取设备标识符（IMEI / MAC / Advertising ID）、不读取位置，不做设备或使用行为的画像采集
- **隐私声明同步**：关于页隐私弹窗文案（`strings.xml` 的 `about_privacy_*`）描述当前真实的数据处理行为，变更出网行为时须同步更新

**历史遗留清理**：`TalkifyApplication.onCreate` 执行一次 `deleteLegacyTelemetryPrefs()`，抹除旧版本遥测写入的 `talkify_tts_telemetry` SharedPreferences 存储；崩溃链路只记录本地日志，不再有任何上报。
...
graph TD
    ...
    DialogBattery -- 去设置/跳过 --> Step4[4. 检查默认供应商状态]
    Step3 -- 无限制 --> Step4
    
    Step4 -- 非默认供应商 --> BannerDefault[显示设为默认供应商横栏]
    Step4 -- 已是默认供应商 --> MainUI[进入主界面]
    
    BannerDefault -- 点击横栏 --> TtsSettings[跳转系统 TTS 设置]
    TtsSettings -- 返回应用 --> Step4[重新检查默认供应商状态]
    
    MainUI -- 应用恢复前台 --> Step4[重新检查默认供应商状态]
```
...
| `Talkify-v<versionName>-universal.apk` | 全 ABI 合集包，供不便区分 ABI 的用户下载 |

- **发布**：GitHub Release 需将各 ABI 包与 universal 包一并上传（应用内不含版本检查，用户需自行到 Release 页面下载）。
